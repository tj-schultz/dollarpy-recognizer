from math import acos, atan, atan2, floor, pi, sin, cos, sqrt, dist, radians, inf, fabs, pi
import path as pth
import dollar
import numpy as np
...
class NDollarRecognizer():
    def __init__(self, useProtractor=False, useBoundedRotationInvariance=False, requireSameNumStroke=False):
    ...
    #
    def recognize(self, strokes, preprocessed=False):
        ...
        if preprocessed: 
            ...
            candidate = Unistroke("", self.useBoundedRotationInvariance, points, preprocessed)
        m = None
        b = inf
        n_best_list = {}
    ...
    def add_gesture(self, name, useBoundedRotationInvariance, strokes):
        pass
...
def optimal_cosine_distance(v1, v2):
    ...
    acos_input = float(a * cos(angle) + b * sin(angle))
    simplify = floor(acos_input/2)
    acos_input = float(acos_input - (2 * simplify))
    print(acos_input)
