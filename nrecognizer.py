...
class Result():
    def __init__(self, name, score, ms, n_best):
        self.name = name
        self.score = score
        self.time = ms
        self.n_best = n_best
...
class Recognizer():
    def __init__(self, useBoundedRotationInvariance=False, requireSameNumStroke=False, useProtractor=False, input_templates=False):
    ...
    def recognize(self, strokes):
        ...
        if m==None or n_best_list[m] >= 1:
            return Result("No match.", 0.0, t1-t0, n_best_list)
        else:
            return Result(m, (1.0 - b) if self.useProtractor else (1.0 - b / HALF_DIAGONAL), t1-t0, n_best_list)
    
    def add_gesture(self, name, strokes):
        multistroke = Multistroke(name, self.useBoundedRotationInvariance, strokes)
